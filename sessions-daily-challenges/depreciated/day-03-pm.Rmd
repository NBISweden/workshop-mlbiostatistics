...
1. Given a linear model $Y_i=\alpha + \beta x_i + \epsilon_i$ could you use outcome, response, dependent variable, exposure, independent variable, predictor and covariate terminology correctly to talk about $Y$ and $x$?

2. Given a linear model $Y_i=\beta_0 + \beta_1 x_{i,1} + \beta_2 x_{i,2}^2 + \epsilon_i$ could you write the design matrix?

3. Given output of lm() regression could you interpret the slope regression coefficients?

4. Given the output of lm() could you perform a hypothesis test to test $H_0: \beta = 0$?

5. Given the output of lm() with categorical gender variable could you predict the response values for a male?

6. In simple linear regression model, could you calculate $R^2$ knowing the response and fitted response values, e.g. 
```{r}
...
7. Could you use residuals to check assumptions of the linear regression model? 

8. Could you interpret the logistic regression coefficients in terms of odds?
...
a) $\begin{bmatrix} 1 & x_1  \\ 1 & x_2  \\ \vdots & \vdots \\ 1 & x_{n} \end{bmatrix}$ b) $\begin{bmatrix} 1 & x_1 & x_2  \\ 1 & x_2 & x_2 \\ \vdots & \vdots & \vdots \\1 & x_n & x_{n}\end{bmatrix}$ c) $\begin{bmatrix} 1 & x_{1,1} & x_{1,2}  \\ 1 & x_{2,1} & x_{2,2} \\ \vdots & \vdots & \vdots \\ 1 & x_{1,n} & x_{n,2} \end{bmatrix}$ d) $\begin{bmatrix} 1 & x_{1,1} & x_{1,2}^2  \\ 1 & x_{2,1} & x_{2,2}^2 \\ \vdots & \vdots & \vdots \\ 1 & x_{1,n} & x_{n,2}^2 \end{bmatrix}$
...
4. Given the output of lm() could a hypothesis test to test $H_0: \beta = 0$. What is the p-value?
a) 0.0013
b) 0.0023
c) 0.3677
d) 0.0001
