...
<br> $$\sigma = \sqrt{\frac{1}{n} \sum_{i=1}^n (x_i - \bar x)^2}$$ {#eq-sigma}
...
<br> and the **sample standard deviation** is calculated as:
...
-   There can be also lines called **whiskers** extending from the rectangle indicating variability outside the upper and lower quartiles. These can be defined in few ways[^1].
...
```{r}
#| label: tbl-gtsummary
#| tbl-cap: "Example of the gtsummary table."
