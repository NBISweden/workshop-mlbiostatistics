...
bookdown::render_book(c("index.Rmd", "probability.Rmd"), output_dir=".", preview=TRUE, bookdown::html_book(lib_dir = "libs", number_sections=TRUE, toc=FALSE, split_by="chapter"))
...
wd <- getwd()
setwd("..")
sapply(list.files("session-probability", "prob-.*html", full.names = TRUE), postpage, token=token, kursid=kursid)

setwd(wd)
##Render book for github
bookdown::render_book(c("index.Rmd", "probability.Rmd"), output_dir=".", preview=TRUE, bookdown::gitbook(lib_dir = "libs"))
