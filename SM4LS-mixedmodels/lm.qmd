...
You could of course fit a separate model for each group, or include group as a `covariate` in the model. Both of these options can be OK in circumstances that we don't have too many groups and we have enough data in each of groups. Below we include cluster as a covariate to see what happens to the coeffcients.
...
To make this even more clear we can have a look at the coefficients of the the model:
