...
- **Censoring** occurs when the event of interest has not yet been observed by the end of the study. This makes survival analysis especially useful for incomplete data sets, where we can still make use of the available information to study time to event patterns.

- With survival analysis, we can compare groups (e.g., treatment vs. control) and determine which factors are associated with an increase or decrease of the hazard of the event occurring over time, even when adjusting for other variables. Here, the **hazard** can be thought of as the instantaneous event rate at a given time among individuals who have not yet experienced the event. Informally, it can be thought of as the risk of the event happening "right now".
...
If the event occurs before censoring (i.e. $T \le C$), then we observe the true survival time $T$. Otherwise, if censoring occurs before the event (i.e. $T > C$), we observe the censoring time $C$.
...
The figure below illustrates the data for 10 patients in this fictitious study.
...
We observe 10 $(Y, \delta)$ pairs, which we denote as $(y_1, \delta_1), \dots, (y_n, \delta_n)$, where $n = 10$. From @fig-censoring, we can see that: 

- for patient #1 we get $(y_1= t_1, \space \delta_1 = 1)$ since we observe event (death) as ca. 23 months, before the end of the follow-up at 24 months. 
- for patient #2 we get $(y_2= c_2, \space \delta_2 = 0)$ since the patient is still alive at the end of the study at 24 months, meaning that the patient has survived at least 24 months (censored)
- for patient #3 we get $(y_3= t_3, \space \delta_3 = 1)$ since we observe event (death) at 9 months
- for patient #4, we get $(y_4= c_4, \space \delta_4 = 0)$, another censored data point. The study is not over yet, the patient has not died but is **lost to follow-up**, e.g. due to withdrawing from the study
...
In this chapter, we will focus on right-censored data. 
...
The **Kaplan-Meier (KM) estimator** is a non-parametric statistic used to estimate the survival function from lifetime data and is given by: $$\hat{S}(t) = \prod_{t_j\le t} \left( 1 - \frac{d_j}{n_j} \right)$$ {#eq-KM}

where: 

- $d_j$: number of events (failures) at time $t_j$
- $n_j$: number of patients at risk just before time $t_j$
- and the product is taken over all event times  up to and including $t$
...
Note that the `status` column is a binary event indicator of survival status (1 = event, 0 = censored). In this example, we assume all patients have experienced the event (failure) and have complete follow-up data.

To calculate the Kaplan-Meier curve:

1. We **sort** the data by time to failure.
2. For each event time, we calculate the **survival probabilities**, i.e. the probability the filling lasting beyond that time.
3. We apply the Kaplan-Meier formula (@eq-KM) to calculate the probability of surviving up to time $t$, as the product of all individual survival probabilities at each time up to time $t$: 
...
- At 4 months: 9 patients are at risk (since one patient failed at 3 months), and the survival probability is now cumulative (0.9 from the first interval multiplied by 0.888 from this interval).
...
| Time<br>(months) | Patients at Risk<br>$n_i$ | Failures<br>$d_i$ | Censored | Survival Probability <br>at $t_i$ | Cumulative Survival<br>$S(t)$ |
|---------------|-----------------------------|--------------------|----------|----------------------------------|--------------------------------|
...
| 8             | 5                           | 2                  | 0        | $1 - \frac{2}{5} = 0.6$      | 0.571 $\times$ 0.6 = 0.343 |
| 10            | 3                           | 0                  | 1        |                                 | 0.343                          |
...
#| fig-cap: Kaplan-Meier survival curve for tooth filling data with censoring. The curve shows the estimated probability of survival (filling intact) over time. Censoring times are indicated with tick marks.
...
If the interest is the **median survival time**, this may be estimated from the survival curve as the time corresponding to a cumulative survival probability of 0.5.
<!-- Alternatively, it can be calculated as: $$\textrm{Life expectancy} = 0.5 + \sum(\textrm{length of interval} \times \textrm{cumulative chance of survival})$$ -->

If there are multiple curves representing different groups, you can compare their shapes and patterns. If the curves are similar, it suggests that the groups have similar survival experiences, Large differences between the curves, diverging or crossing curves, suggest differences in survival between the groups, although formal inference requires an appropriate statistical test.
...
The differences between the survival curves are not constant over time. For example, both groups start at 1 and the survival probability decreases, with lower survival probability for men at 10 months, but then again higher survival probability at 15 months. 

One common way to compare groups is through their **hazards** over the duration of the follow-up. In the Cox proportional hazards framework, we assume that the **ratio of the hazards** in the two groups remains constant over time, even if the underlying hazards change. That is, at all times $t$:
...
This assumption is called the **proportional hazards assumption**.

It is difficult to estimate the hazard directly from the data, since this would give a series of "spikes" when an even occurs, interspersed with zeros when there is no disease event. Instead we use the **cumulative hazard function**, $H(t)$. This is the total hazard experienced up to time $t$, and is estimated by
$$H(t) = \sum_{t_j \le t}\frac{d_j}{n_j}$$
...
Therefore, if the proportional hazards assumption hold, then graphs of the log of the cumulative hazard functions should be approximately parallel.
...
Let's start with two groups, A and B. If at time $t_j$ there were $d_j$ events, and there were $n'_{jA}$ and $n'_{jB}$ subjects at risk just before $t_j$ in groups A and B respectively, then the data can be arranged in a $2\times2$ table: 
