...
1. $0 \leq P(E) \geq 1$ for any event $E \subseteq S$
2. $P(S) = 1$
3. if $E$, $F$ are disjoint events, then $P(U \cup F) = P(E) + F(J)$
...
Probability density functions for the binomial, hypergeometric and Poisson distributions can in R can be computed using functions `dnorm`, `dt`, `dchisq`, and `df`, respectively.
...
Useful R functions: `ppois`,  `dpois`, `qpois`
