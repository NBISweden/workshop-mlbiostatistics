# Appendix: cotinuous random variables {.unnumbered}
...
plot(ggplot(df.nwt, aes(x=w, y=F)) + geom_line() + xlab("x") + ylab("F(x)") + theme_bw() + geom_point(data=data.frame(w=4, F=pnorm(4,3.5,.5))) + annotate("label",label=sprintf("F(4.0)=%.2f", pnorm(4,3.5,.5)), x=4, y=.84, hjust=-0.2))##+ ggtitle("Cumulative distribution function") 
```
...
#| fig.width: 5
#| fig.height: 5
rs <- data.frame(n = c(3,5,10,15,20,30)) |> group_by(n) |> reframe(m=replicate(10000, mean(sample(miceage, n))), .groups="drop")
...
## Chi-square distribution {pdf-title="Chi-Square distribution"}
