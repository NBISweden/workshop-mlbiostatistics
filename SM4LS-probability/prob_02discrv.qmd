# Appendix: discrete random variables {.unnumbered}
...
When rolling a die there are six possible outcomes: 1, 2, 3, 4, 5 and 6, each of which has the same probability, if the die is fair. The outcome of one die roll can be described by a random variable $X$. The probability of a particular outcome $x$ is denoted $P(X=x)$ or $p(x)$.
...
tibble(x = 1:6, `p(x)` = round(rep(1/6, 6), 3)) |> gt()
...
The number of bacterial colony-forming units (CFU) on a plate is a random variable that can be described by a PMF such as the one shown in @fig-CFU.
...
$$var(X) = \sigma^2 = E[(X-E[X])^2] = \sum_{x \in S} (x-\mu)^2 p(x).$$ 
...
In a single coin toss, the probability of heads is 0.5. In 20 coin tosses, what is the probability of at least 15 heads?
...
Count how many times out of our `r length(Nheads)` experiments, the number of heads is 15 or greater.
...
```{r pbernoulli, eval=FALSE, echo=FALSE}
tibble(x=c(0,1), "p(x)"=c("1-p", "p")) |> gt()
