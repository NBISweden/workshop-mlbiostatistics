...
- Therefore, there is no evidence that the relationship between BMI and height differs between men and women.
- However, when we plot the fitted model, the fitted lines are not exactly parallel. The estimated interaction is non-zero, but the data do not provide sufficient evidence that this difference is different from zero.
