...
## The linear model describes an expected value
...
This means that for individuals with the same value $X=x$, their observed outcomes $Y$ do not all have to lie exactly on the regression line. Instead, the line describes their expected, or average, outcome.

An individual observation can therefore be written as

$$
Y_i = E(Y_i\mid X_i=x_i) + \epsilon_i
$$

or, for the linear model,

$$
Y_i = \alpha+\beta x_i+\epsilon_i.
$$

The error term $\epsilon_i$ represents how much an individual observation differs from its expected value.

When we fit the model from a sample, we estimate this expected value by

$$
\hat{Y}_i=\hat{\alpha}+\hat{\beta}x_i,
$$

and the difference between the observed and fitted value is the **residual**:

$$
e_i=Y_i-\hat{Y}_i.
$$

Thus, the regression line describes how the **expected outcome changes with the predictor**, while individual observations vary around that expectation.
...
## From simple regression to more general linear models

So far, we have considered **simple linear regression**, where a continuous outcome $Y$ is described using one numerical predictor $X$:

$$
Y_i = \beta_0 + \beta_1 x_i + \epsilon_i.
$$

The same framework can be extended to include **more than one predictor**. For example, with two predictors:

$$
Y_i = \beta_0 + \beta_1 x_{1i} + \beta_2 x_{2i} + \epsilon_i.
$$

More generally, a linear model with $p$ predictors can be written as:
...
Each coefficient describes the contribution of one predictor to the expected outcome, **while keeping the other predictors in the model constant**.

For example, if body weight is modelled using height and age,

$$E(\text{weight}\mid\text{height},\text{age})=
\beta_0 +
\beta_1\text{height} +
\beta_2\text{age},
$$

then $\beta_1$ describes the expected difference in weight associated with a one-unit difference in height **for individuals of the same age**.

Predictors in a linear model do not have to be numerical. They can also be **categorical**, such as treatment group, sex, genotype or experimental condition. This allows the same linear-model framework to describe both relationships between continuous variables and differences between groups.

We will look at how numerical and categorical predictors, and combinations of several predictors, are used in practice in the next session.
...
## Overall model test: the F-test

The `summary()` output also reports an **F-statistic** and its corresponding
*P*-value. The F-test evaluates the **linear model as a whole**.

For a linear model with several predictors,

$$
Y_i = \beta_0 + \beta_1x_{1i} + \cdots + \beta_{p-1}x_{(p-1)i} + \epsilon_i,
$$
...
In other words, under the null hypothesis none of the predictors contributes
to explaining variation in the response, and the model reduces to an
intercept-only model.

The **F-statistic** compares the variation explained by the regression model
with the variation left unexplained by the model:

$$
F =
\frac{\text{explained variation per degree of freedom}}
     {\text{unexplained variation per degree of freedom}}.
$$
...
number of estimated model parameters, including the intercept.

For our plasma-volume example,
...
Here we have $n=8$ observations and $p=2$ model parameters, the intercept
$\alpha$ and slope $\beta$. Therefore,

$$ F \sim F_{1,6} $$

under the null hypothesis.

The P-value is the probability, under $H_0$, of observing an F-statistic at
least as large as the one obtained:

$$ P(F_{1,6} \geq 8.16) \approx 0.029. $$

Thus, there is sufficient evidence to reject the null hypothesis and conclude
that the model containing weight explains more variation in plasma volume
than an intercept-only model.

In simple linear regression, with only one predictor, the overall F-test
tests exactly the same hypothesis as the t-test for the slope:
...
so the two tests give the same P-value. With several predictors, the F-test
is useful because it tests whether the predictors are jointly associated
with the response.
...
The following simulated examples show some typical patterns that may appear
in diagnostic plots when linear model assumptions are violated.
...
- **Non-linearity:** a systematic curved pattern remains in the residuals.
- **Non-constant variance:** the spread of residuals changes with the fitted values, often forming a funnel shape.
- **Non-normality:** residuals deviate systematically from the straight line in the normal Q--Q plot.
- **Non-independence:** neighbouring observations tend to have similar residuals, producing runs or other structure when residuals are plotted in observation order.

The exact appearance will vary between data sets. Diagnostic plots should therefore
be interpreted as tools for identifying **systematic patterns**, rather than as
formal pass/fail tests.
