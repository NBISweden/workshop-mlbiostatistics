...
plot(x_celcius, y_fahr, type="b", pch=19, xlab="Celcius", ylab="Fahrenheit", main="Deterministic", cex.main=1, las=2)
```
...
plot(x, y_random, pch=19, xlab="x", ylab="y", main="Random", cex.main=1, las=2)
```
...
plot(x, y_increasing, pch=19, xlab="x", ylab="y", main="Statistical", cex.main=1, las=2)

# Statistical relationship (decreasing)
y_decreasing <- -2*x + rnorm(length(x), mean=100, sd=25)
plot(x, y_decreasing, pch=19, xlab="x", ylab="y", main="Statistical", cex.main=1, las=2)
...
-   In addition to knowing how to estimate model parameters
-   we need to learn to assess the goodness of fit of a model.
...
# Common cases
- Let's go over together through common cases
