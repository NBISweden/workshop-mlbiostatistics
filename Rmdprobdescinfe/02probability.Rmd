...
knitr::opts_chunk$set(fig.width=3.5, fig.height=3.5, echo = FALSE, error=FALSE, warnings=FALSE, dpi=600, fig.path = "Rfigures/")
options(digits=2)
```
...
# Exercises: Discrete random variables {.unnumbered #prob-exercise-discrv}
...
```{r pmfchildren}
kable(matrix(c(as.character(0:6),c("0.14","0.20","0.27","0.19","0.13","0.05","0.02")),ncol=7, byrow=TRUE, dimnames=list(c('x','p(x)'), c()))) %>% kable_styling(full_width = FALSE)
```
...
For a sinle patient, what is the probability of being assigned to
  a) the treatment group?
  b) the control group?

If 20 patients are enrolled in the study;

  a) what is the probability of exactly 15 in the patient group?
  b) what is the probability of less than 7 in the patient group?
  c) What is the most probable number of patients in the treatment group?
  d) what is the probability of 5 or less patients in the control group?
  e) what is the probability of 2 or less patients in the treatment group?
```
...
# Exercises: Continuous random variables {.unnumbered #prob-exercise-contrv}
...
The lower and upper critical values of activbe substance should be 11.02 and 12.98 mg.
...
# Exercises: Random sample {.unnumbered #prob-exercise-sample}
