...
Unknown population parameters can be inferred from estimates from samples from the population  of interest. The sample estimate will be our best guess, a **point estimate**, of
...
Based on this observation our point estimate of the Uppsala popultation proportion $\pi$ is $\pi \approx p = 0.42$. There is an uncertainty in this measurement, if the experiment is repeated 100 other persons would be selected and our point estimate would be slightly different.
:::
...
The expected value of an **unbiased** point estimate is the the population parameter that it estimates, This means that if you would repreat the sampling many times and compute the point estimate of interest, e.g. sample mean, the average of sample means would be the population mean.

```{r}
#| label: bioasprecision
#| fig-cap: "Bias and precision."
bias <- c(Biased=1.5, Unbiased=0)
precision= c(Precise=0.5, Unprecise=2)
df <- data.frame(B=rep(names(bias), each=2), P=rep(names(precision),2)) |> mutate(m=bias[B], s=precision[P]) |> group_by(B, P) |> summarize(x=rnorm(10, mean=m, sd=s), y=rnorm(10, mean=m, sd=s)) 

ggplot() + geom_point(data=df, aes(x=x, y=y, color=B, shape=P)) + facet_grid(B~P) + geom_circle(data=data.frame(x0=0, y0=0,r=1:6), aes(x0=x0,y0=y0,r=r)) + theme_classic() + theme(panel.grid=element_blank(), axis.ticks  = element_blank(), axis.text=element_blank())
```

Even an unbiased point estimate is not perfect, it will have a cettain amount of uncertainty.
...
```{r fig-samplingdistr, out.width="50%", fig.cap="The sampling distribution of $\theta$. The probability that the observed value of a random sample falls in the "}
w<-seq(-3.5,3.5,.01)
