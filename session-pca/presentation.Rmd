...
```{r nci_complex-text-table, fig.dim=c(20, 7), fig.cap=fig.cap}
...
```{r dispersion, fig.cap = fig.cap, fig.dim=c(8, 4)}
# fig.cap <- paste(
...
```{r dispersion_correlated, fig.cap = fig.cap, fig.dim=c(8, 5)}
# fig.cap <- paste(
#   "The red numbers show the index of each sample.", 
#   "The red line shows the mean and the dashed lines show the distance between each observation and mean."
# )
...
.pull-left[
```{r correlated-scatter, fig.dim=c(7, 7)}
...
$\begin{equation}\underset{n\times 1}{\mathbf{y}_1} = \left[ {\begin{array}{c} y_{11} \\ y_{21} \\ \vdots \\ y_{n1} \end{array}} \right] , \quad \underset{n\times 2}{\mathbf{X}} = \left[ {\begin{array}{cc} x_{11} & x_{12} \\ x_{21} & x_{22} \\ \vdots & \vdots \\ x_{n1} & x_{n2} \\ \end{array}} \right] \quad and \quad \underset{2\times 1}{\mathbf{a}_1} = \left[ {\begin{array}{c} a_{11} \\ a_{21} \\ \end{array}} \right]\end{equation}$
...
.pull-right[
```{r pca_2d-pt_on_new_axis, fig.dim=c(7, 7)}
...
.pull-right[
```{r pca_2d-pt_on_new_axis2, fig.dim=c(7, 7)}
# coordinates after projection onto PC1
x <- pca$x
x[, 2] <- 0   # PC2 = 0
on_pc1 <- t(t(x %*% solve(pca$rotation)) + pca$center)
...
```{r pca_2d-1d_plot, fig.dim=c(7, 2.6)}
...
$\begin{equation}\hat{\lambda}_1\end{equation}$
, $\hat{\mathbf{e}}_1$
) and (
$\begin{equation}\hat{\lambda}_2\end{equation}$
, $\hat{\mathbf{e}}_2$) where 
$\begin{equation}\hat{\lambda}_1 \ge \hat{\lambda}_2 \ge 0\end{equation}$
...
As the total sample variance is $\begin{equation}\sum_{k} \hat{\lambda}_k\end{equation}$,
...
$\begin{equation} \underset{n\times p}{\mathbf{X}} = \left[ {\begin{array}{cccc} x_{11} & x_{12} & \cdots & x_{1p} \\ x_{21} & x_{22} & \cdots & x_{2p} \\ \vdots & \vdots & \ddots & \vdots \\ x_{n1} & x_{n2} & \cdots & x_{np} \\ \end{array}} \right] \quad and \quad \underset{p\times 1}{\mathbf{a}_1} = \left[ {\begin{array}{c} a_{11} \\ a_{21} \\ \vdots \\ a_{p1} \\ \end{array}} \right] \end{equation}$
...
, $\hat{\mathbf{e}}_1$), (
$\hat{\lambda}_2$
, $\hat{\mathbf{e}}_2$), $\cdots$, (
$\hat{\lambda}_p$
, $\hat{\mathbf{e}}_p$) where $\hat{\lambda}_1 \ge \hat{\lambda}_2 \ge \cdots \ge \hat{\lambda}_p \ge 0$,
...
```{r pca_score_plot-basic, fig.dim=c(7, 7), out.width="55%"}
opa <- par(mar = c(5, 4, 0, 2) + 0.1)
...
```{r pca_score_plot, fig.dim=c(7, 7), out.width="55%"}
library(ggfortify)
autoplot(pca)
```
...
```{r bi_plot, fig.dim=c(7, 7), out.width="55%"}
