...
Sometimes two observations are naturally linked, rather than coming from two independent groups. This happens, for example, in before-and-after studies where the same subjects are measured twice, or in matched study designs where each individual in one group is paired with a similar individual in the other group.
...
In a paired design, the analysis should take the pairing into account. Treating paired observations as independent would ignore important information and usually reduce the power of the test.

The null hypothesis in a paired analysis is usually that the mean difference within pairs is zero.

Let the paired observations be $(X_{i1}, X_{i2})$ for $i=1,2,\dots,n$, and define the within-pair differences

$$D_i = X_{i2} - X_{i1}.$$

This can now be handled as a one sample test based on the diffewrence and the hypotheses can then be written as
...
If a one-sided question is of interest, the alternative hypothesis could instead be

$$H_1: \mu_D > 0 \quad \text{or} \quad H_1: \mu_D < 0.$$

The paired t-test is simply a one-sample t-test applied to the differences. If the differences are approximately normally distributed, then the test statistic

$$T = \frac{\bar D}{s_D/\sqrt{n}}$$

is t-distributed with $n-1$ degrees of freedom, where $\bar D$ is the sample mean difference and $s_D$ is the sample standard deviation of the differences.
