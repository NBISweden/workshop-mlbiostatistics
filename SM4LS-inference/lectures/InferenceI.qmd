...
:::
:::{.column width="92%"}
A **hypothesis test** is a type of inference used to evaluate if a hypothesis about a population is supported by the observations of a random sample (i.e by the data available).
:::
::::
...
## A first hypothesis test example

In Sweden as a whole, about **30%** of people are allergic to pollen.

We observe **100 people in Uppsala**, and **42** of them are allergic to pollen.

**Question:**  
Is there reason to believe that the true proportion in Uppsala is greater than 0.3?

. . .

This is a typical **hypothesis testing** question.

## Null and alternative hypotheses

Let $\pi$ denote the true proportion of pollen allergy in the Uppsala population.

We want to investigate whether the proportion in Uppsala is greater than in Sweden as a whole.
...
- $H_0$: no increase in Uppsala
- $H_1$: higher proportion in Uppsala


## Observed statistic

A simple summary of the sample is the **sample proportion**:
...
- $x$ = number allergic in the sample
- $n$ = sample size
...
$$
p_{obs}=\frac{42}{100}=0.42
$$
## What would we expect if $H_0$ were true?

If $H_0$ is true, then the pollen allergy proportion in Uppsala is also 0.30.

Then a sample of 100 people from Uppsala should behave like a random sample where
...
**If the true proportion is 0.30, how unusual is an observed sample proportion of 0.42?**

## Simulate under $H_0$ in R

We can simulate many samples of size 100 under the null hypothesis.

```{r echo=TRUE}
set.seed(13)

pnull <- replicate(10000, {
  x <- sample(c(0, 1), size = 100, replace = TRUE, prob = c(0.7, 0.3))
  mean(x)
})
```

Each simulated value is a sample proportion from a world where $H_0$ is true.
...
This is an estimated null distribution of the sample proportion.
...
#| fig-align: center
#| out-width: 70%
ggplot(data.frame(p = pnull), aes(x = p)) +
  geom_histogram(binwidth = 0.02, color = "white") +
  geom_vline(xintercept = 0.42, color = "red") +
  theme_bw() +
  xlab("Sample proportion") +
  ylab("Frequency")
```

The red line marks the observed value, $p_{obs}=0.42$.


## p-value

The **p-value** is the probability of observing the value we got, or something even more extreme, **if $H_0$ is true**.

Here:

$$
p\text{-value}=P(P\geq 0.42\mid H_0)
$$

Estimate it from the simulated null distribution:
...
If this probability is very small, then the observed result would be unusual under $H_0$.


## Decision

Suppose we use the significance level
...
Here, the p-value is small, so we reject $H_0$.

**Conclusion:**  
The data provide evidence that the proportion of pollen allergy in Uppsala is greater than 0.30.
...
1. Defined a **null hypothesis** and an **alternative hypothesis**
2. Computed an **observed statistic**
3. Simulated the **null distribution**
4. Computed a **p-value**
5. Reached a conclusion by comparing the p-value to $\alpha$

. . .

This is the basic structure of a **hypothesis test**.

## We were slightly sloppy...

In our first example, we introduced the main idea of a hypothesis test:

- compare the observed result to what we would expect under $H_0$
- compute a p-value
- decide whether the result is surprising under $H_0$

. . .

But we skipped one important step:

**the significance level, $\alpha$, should be chosen before looking at the result.**

To perform a hypothesis test properly, we should follow a more careful recipe.
...
4.  Assume that $H_0$ is true and compute the sampling distribution of $T$.

::: {.notes}
We will get back to the null distribution in the next slide
:::

5.  Compare the observed value, $t_{obs}$, with the computed sampling distribution under $H_0$ and compute a p-value.
...
If high-fat diet has no effect, i.e. if $H_0$ were true, the result would be as if all mice were given the same diet.
...
2.  Compute the difference in mean weights.

If we repeat steps 1 and 2 many times, we get the sampling distribution of the mean difference when $H_0$ is true, that is, the null distribution.
...
What is the probability to get an at least as extreme mean difference as our observed value, $d_{obs}$, if $H_0$ were true?
...
$$P(\bar X_2 - \bar X_2 \geq d_{obs} \mid H_0) = `r sprintf("%.3g",mean(dnull.perm>=dobs))`$$
...
If $\sigma$ is known, then $Z\sim N(0,1)$.
...
Two independent samples are collected (one from each population) and the means are compared. This can for example be used to determine if a treatment group differs from a control group in terms of the mean of a property of interest.
...
$$
\begin{aligned}
\bar X_1 \sim N\left(\mu_1, \frac{\sigma_1^2}{n_1}\right) \\
\bar X_2 \sim N\left(\mu_2, \frac{\sigma_2^2}{n_2}\right)
\end{aligned}
$$
...
$$
D = \bar X_2 - \bar X_1 \sim N\left(0, \frac{\sigma_2^2}{n_2} + \frac{\sigma_1^2}{n_1}\right)
$$

The test statistic:

$$Z = \frac{\bar X_2 - \bar X_1}{\sqrt{\frac{\sigma_2^2}{n_2} + \frac{\sigma_1^2}{n_1}}}$$ 
is standard normal, i.e. $Z \sim N(0,1)$.
...
If the sample sizes are large, we can replace the population standard deviations by the sample standard deviations and, according to the central limit theorem, assume that 
...
If equal variances can be assumed, Student's t-test can be used; otherwise Welch's t-test for unequal variances is used.
...
Assume equal variances in the two populations and compute the pooled sample variance:
...
For unequal variances the test statistic is

$$t = \frac{\bar X_2 - \bar X_1}{\sqrt{\frac{s_2^2}{n_2} + \frac{s_1^2}{n_1}}}$$
$t$ is $t$-distributed and the degrees of freedom are approximated using Welch's method.

The degrees of freedom are approximated using Welch's method.

Fortunately, the t-test is implemented in R, for example in the function `t.test` in the `stats` package. This function can compute both Student's t-test with equal variances and Welch's t-test with unequal variances.
...
- The significance level should be chosen before looking at the result.
