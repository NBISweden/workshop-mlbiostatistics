...
precision= c(Precise=0.5, Imprecise=2)
...
:::{.notes}
The expected value of an **unbiased** point estimate is the population parameter that it estimates. This means that if you would repeat the sampling many times and compute the point estimate of interest, e.g. sample mean, the average of the sample means would be the population mean.
:::
...
- An interval estimate is an interval of plausible values for the population parameter.
...
A confidence interval is a range of plausible values for an unknown population parameter $\theta$.

More precisely, a confidence interval for $\theta$ with confidence level $1-\alpha$ is a method for constructing intervals such that, in repeated sampling, a proportion $1-\alpha$ of the intervals will contain the true population parameter $\theta$.
...
lbl <- sprintf("%iSE", -3:3)
lbl[4]=expression(mu)
pl <- ggplot(df.nwt, aes(x=x, y=f)) + geom_line() + theme_bw() + scale_x_continuous(breaks=-3:3, labels=lbl) + xlab("")
pl
```
...
pl2 <- ggplot(data.frame(x=0), aes(x=x, y=0)) +
  ...
  scale_color_manual(values = unname(cols)) + geom_hline(yintercept = 0) + 
...
Use what is known about the sampling distribution and compute a confidence interval around $m$.
...
* 95% confidence, $\alpha = 0.05$ and $z_{\alpha/2} = z_{0.025}=1.96$
* 90% confidence, $\alpha = 0.10$ and $z_{0.05} = 1.64$
* 99% confidence $\alpha = 0.01$ and $z_{0.005}=2.58$
:::
::::
...
Replace $\bar X$ by the observed sample mean, $\bar x$, to get the $1-\alpha$ confidence interval:

$$[\bar x - z_{\alpha/2}SEM, \bar x + z_{\alpha/2}SEM]$$
...
The mean of a sample of $n$ independent and identically distributed normal observations $X_i$ is normally distributed;
...
The confidence interval with confidence level $1-\alpha$ is;
...
## Example: Confidence interval for mean BMI
...
* Sample mean $\bar x = 27.3$
* Sample standard deviation $s = 3.39$
* Degrees of freedom $n-1 = 5$
* $t$-value for a 95% confidence interval is `r qt(0.975, df=5)`.
...
```{r CIbootmean}
#| fig-cap: "95% bootstrap interval."
#| out-width: "70%"
#| fig-width: 7
#| fig-height: 5
#| fig-align: "center"
ggplot(data.frame(mboot = mboot), aes(x = mboot)) +
  geom_histogram(binwidth = 0.5, fill = "grey75", color = "white") +
  geom_vline(xintercept = ciboot, linetype = 2, color = "black", linewidth = 1) +
  geom_vline(xintercept = mean(x), color = "black", linewidth = 1) +
...
df <- bind_rows(df1 |> mutate(type="Large within-group variation"), df2 |> mutate(type="Small within-group variation"))
...
If the assumptions of one-way ANOVA are not met, a rank-based alternative is the **Kruskal–Wallis test**. This test compares the groups using the ranks of the observations rather than the raw values.
...
$$\alpha = P(\textrm{type I error}) = P(\textrm{false alarm}) = \\ P(\textrm{Reject }H_0 \mid H_0 \textrm{ is true})$$
$$\beta = P(\textrm{type II error}) = P(\textrm{miss}) = \\ P(\textrm{Fail to reject }H_0 \mid H_1 \textrm{ is true})$$
...
- Benjamini-Hochberg adjusted p-values are often called **q-values**.

  After sorting the p-values so that $p_1 \le p_2 \le \dots \le p_m$, the BH adjusted p-values are computed from

  $$
  q_i = \min\left(1,\; \min_{j \ge i}\frac{m}{j}p_j\right).
  $$

  A feature's q-value can be interpreted as the lowest FDR at which the corresponding null hypothesis would be rejected.
