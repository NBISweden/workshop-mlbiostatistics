...
| Study design | Parametric test | Rank-based alternative |
|---|---|---|
| One sample | One-sample t-test | Wilcoxon signed-rank test |
| Two independent groups | Two-sample t-test | Mann–Whitney / Wilcoxon rank-sum test |
| Paired samples | Paired t-test | Wilcoxon signed-rank test |
| More than two independent groups | One-way ANOVA | Kruskal–Wallis test |
...
If $m$ independent tests are performed, the risk of at least one false alarm increases;
...
- Benjamini-Hochberg adjusted p-values are often referred to as q-values:
...
To show this uncertainty, an **interval estimate** for a population parameter can be computed instead of reporting only a point estimate.
...
plz <- ggplot(df, aes(x=x, y=f)) + geom_line() + theme_bw() + xlab("z") + ylab("f(z)") + scale_x_continuous(breaks=c(-1.5,0,1.5), labels=c("-z","0","z")) +
     theme(panel.grid.minor = element_blank(),
              panel.grid.major.x = element_line(color = c("grey92", NA, "grey92", NA, "grey92")))
plz  + geom_area(data=df %>% filter(x>(-1.5) & x<1.5)) + annotate("label",label=TeX("$P(-z_{\\alpha/2} \\leq Z \\leq z_{\\alpha/2}) = 1 - \\alpha$"), x=0, y=0.11, hjust=0.5)
```
:::
::::
...
plz <- plz + geom_area(data=df %>% filter(x<(-1.5))) + annotate("label",label=TeX("$P(Z \\leq -z)=\\alpha/2$"), x=-1.6, y=0.07, hjust=1, size=3) + geom_area(data=df %>% filter(x>1.5)) + annotate("label",label=TeX("$P(Z \\geq z) = \\alpha/2$"), x=1.6, y=0.07, hjust=0, size=3)
plot(plz)
```
:::
:::{.column width="50%"}
...
For a 95% confidence interval, $\alpha = 0.05$, and $z_{\alpha/2} = 1.96$. For 90% or 99% confidence $z_{0.05} = 1.64$ and $z_{0.005}=2.58$. 
:::
::::
...
If $\sigma$ is unknown and the observations are approximately normally distributed, use the t distribution instead.

$$T = \frac{\bar X - \mu}{\frac{s}{\sqrt{n}}} \sim t(n-1)$$

It follows that the confidence interval is
...
For a 95% confidence interval and $n=5$, the critical value is `r qt(.975, df=4)`.
