...
require(ggdendro)
require(tidyverse)
knitr::opts_chunk$set(fig.width=3.5, fig.height=3.5, echo = FALSE, error=FALSE, warnings=FALSE, dev="png", dpi=600, fig.path = "figures/")
options(digits=2)
```
...
# K-means {#clust-02kmeans}

The K-means algorith aims to divide all objects into exactly $K$ clusters. $K$ has to be given to the algorithm. The K-means algorithm minimize the variance within clusters, by iteratively assigning each object to the cluster with the closest mean (centroid).
...
$d_i = \sum_{j=1}^p (x_{ij} - m_{kj})^2$. By assigning each object to the closest centroid the total within cluster sum of squares (WSS) is minimized.
...
```{r elbow, fig.height=4, fig.width=7, fig.caption="WSS as a function of the number of clusters, $k$.", warning=FALSE}
library(factoextra)
...
As WSS is constantly decreasing for increasing $k$, we can't use the minimum to find optimal $k$. Eyeballing as in the elbow method is one option, but another is to use the gap statistic that measure the difference between the observed $WSS$ and an expected $WSS$ under a reference (null) model.
...
Choose $k$ as the smallest $k$ such that 
...
The silhoutette value for a single object $i$ is a value between -1 ans 1 that measure how similar the object is to other objects in the same cluster as compared to how similar it is to objects in other clusters.

The silhouette value, $s(i)$, for an object $i$ in cluster $C_a$ is calculated as follows;

1. Calculate the average distance between object $i$ and all other objects that belong to the same cluster $C_a$;
$$a(i) = \frac{1}{|C_a|-1} \sum_{j \neq i, j \in C_a} d(i, j)$$
2. Calculate the average distance between object $i$ and all objects that belong to another cluster $C_b$, $d(i,C_b)$ and define the minimum;
$$b(i) = \min_{b \neq a} d(i, C_b)$$
3. The Silhouette index is defined as;
$$s(i) = \frac{b(i) - a(i)}{\max(a(i), b(i))}$$
A silhouette value close to 1 means that the object is very well clustered, a value close to 0 means that the object lies between two clusters. A negative silhouette value means means that the object is incorrectly clustered.
...
```{r silhouette, fig.height=4, fig.width=7, fig.caption="WSS as a function of the number of clusters, $k$."}
asil <- sapply(2:10, function(k) kmeans(df[, c('x','y')], centers=k)$tot.withinss)
#data.frame(k=1:10, WSS=wss) %>% ggplot(aes(k, WSS)) + geom_point() + geom_line() + theme_bw() + scale_x_continuous(breaks=1:10)
```
### Compute optimal $k$ in R

Several packages in R implement metjhods for choosing optimal number of clusters, one of them is `factorextra`and the `fviz_nbclust` function.


# Dissimilarity matrix {#clust-03dissimilarity}
...
# Hierarchical clustering {#clust-04hierarchical}
...
# Exercises: Clustering {.unnumbered #clust-exercises}
...
df <- data.frame(klass=rep(1:5, c(30, 15, 25, 10,18)), stringsAsFactors = FALSE) %>% mutate(group=letters[klass], x=rnorm(n(),x[klass], sd=sx[klass]), y=rnorm(n(), y[klass], sd=sy[klass])) %>% dplyr::select(-klass)
...
f) Download the R-package `factoextra` and use the function `fviz_nbclust` to compute gap statistic and silhouette width and choose optimal $k$.
...
```{r, echo=TRUE}
wss <- sapply(1:10, function(k) kmeans(df[, c('x','y')], centers=k, algorithm ="Forgy", nstart=4)$tot.withinss)
