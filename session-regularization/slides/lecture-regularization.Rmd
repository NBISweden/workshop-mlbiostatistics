...
```{r,echo=T, dpi=600, fig.height=4, echo=TRUE, eval=FALSE}
library(stats)
ll= vector()
for(i in seq(1,2)){
...
--
```{r test1, dpi=600,fig.height=4, echo=TRUE, eval=FALSE}
rl= vector() 
for(i in seq(1,P)){
...
```{r, echo=T, dpi=600, fig.height=5, eval=FALSE} 
plot(fit, xvar="lambda",label=T)
```


```{r, echo=FALSE, dpi=600, fig.height=4} 
par(mfrow=c(1,1))
par(mar = c(4, 4, 0.1, 0.1)) 
...
```{r, echo=FALSE,dpi=600, fig.height=3}
library(glmnet)
par(mfrow=c(1,1))
# run lasso (alpha=1) for linear model (family=gaussian)
...
names(coefglm)=c("beta","value (oracle)", paste0("estimate(lambda=",signif(minlambda,2),")"))
