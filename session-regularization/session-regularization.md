...
Likelihood builds on the intuition that if $\theta$ (and the model) is close to the 'truth', then $Pr[Y| X, \theta]$ will be higher than for wrong $\theta$ (and model). We should there fore select the $\theta$ that maximizes $Pr[Y| X, \theta]$; this is called maximum likelihood estimation of $\theta$.

Since statistical model contain an element of randomness, this might not always be the case for any single obeservation. However, if we sum over a large number of observations it will be true on average. Hence the need for datasets that are large enough.
...
In maximum likelihood estimation of some parameters $\theta$, one simply selects the estimates $\hat\theta$ that gives the highest likelihood $max_{\theta}L[\theta|X,Y] = L[\hat\theta|X,Y]$.
...
* The Akaike information criterion (AIC), for a model $m$ with variables $X$, is defined as
  $$AIC_m = 2\# X - 2\log \max L[\boldsymbol{\beta}|X,Y]$$
    + We see that $AIC_m = -2 \left(\log \max L[\boldsymbol{\beta}|X,Y] - \#X\right)$, i.e., $-2$ times the log of the simple $rL$, we just looked at. 
...
* Other common notation for LASSO:
  + $$min_{\boldsymbol{\beta}}\left\{RSS\right\} \textrm{subject to} ||\boldsymbol{\beta}||_1 <= t$$
...
* Alternatives to LASSO, differing mainly in the auxiliary criterion
  ...
  - *Elastic-net*, which uses a mixed model combination of the  $L_1$ norm and the $L_2$ norm.
